const {
  ...
  const API_KEY = "AIzaSyDqty3dWZZZ7VgO45FmTAxfC1Y7mPfdS0Y";
  
  async function runai(promp) {
    ...
    const parts = [
      ...
      {text: "input: "+`${JSON.stringify(promp)}` },
      {text: "output: "},
    ];
    console.log("tyru"+`${promp}`)
    ...
    return (JSON.parse(response.text()).suggestion)
  }
